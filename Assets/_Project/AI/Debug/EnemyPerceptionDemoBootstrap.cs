...
{
    /// <summary>
    /// Sandbox-only perception demo owner.
    /// In the Editor it authors a visible enemy prefab/scene instance for debugging.
    /// At runtime it binds that authored instance to the current player and keeps the
    /// existing perception/navigation showcase behavior isolated from production AI setup.
    /// </summary>
    public static class EnemyPerceptionDemoBootstrap
    {
        ...
        private static void InitializeDemoEnemy()
        {
            ...
            {
                Debug.LogWarning(
                    "[AI Perception Demo] No scene-authored Enemy_01 was found. " +
                    "Open TacticalEcho_Sandbox in the Editor or use Tactical Echo/AI/Rebuild Sandbox Perception Enemy.");
                return;
            }
            ...
            {
                Debug.LogError(
                    "[AI Perception Demo] Scene-authored enemy setup is incomplete. " +
                    "Use Tactical Echo/AI/Rebuild Sandbox Perception Enemy to repair the sandbox demo.",
                    enemy);
                return;
            }
            ...
            TMP_Text statusText = labelRoot != null
                ? labelRoot.GetComponentInChildren<TMP_Text>(true)
                : null;
            ...
            if (!enemyOnNavMesh)
            {
                Debug.LogWarning(
                    ...
                    "The authored enemy remains visible/debuggable, but movement will stay disabled. " +
                    "Verify that nearby walkable ground has a Collider and is included in the runtime NavMesh volume.",
                    enemy);
            }

            Debug.Log(
                "[AI Perception Demo] Initialized scene-authored Enemy_01. " +
        ...
        private static bool IsSandboxScene(Scene scene)
        {
            return scene.IsValid() && scene.isLoaded && scene.name == SandboxSceneName;
        }

        private static PlayerVisualController FindPlayerVisual(Scene scene)
        {
            PlayerVisualController[] players = Object.FindObjectsByType<PlayerVisualController>(
        ...
        {
            EnemyPerceptionDemoView[] views = Object.FindObjectsByType<EnemyPerceptionDemoView>(
        ...
        private static void RebuildSandboxEnemyAuthoring()
        {
            ...
            Debug.Log(
                $"[AI Perception Demo] Rebuilt {EnemyPrefabPath} and placed {EnemyInstanceName} in {SandboxSceneName}.");
        }

        private static void OnEditorSceneOpened(Scene scene, OpenSceneMode mode)
        {
            if (!EditorApplication.isPlayingOrWillChangePlaymode && IsSandboxScene(scene))
            {
                EditorApplication.delayCall += () => EnsureSandboxAuthoring(scene);
            }
        }
        ...
        private static void EnsureSandboxAuthoring(Scene scene)
        {
            ...
            {
                Debug.Log(
                    "[AI Perception Demo] Enemy_01 was added to the sandbox, but the scene already had unsaved changes. " +
                    "Save the scene manually when those changes are ready.");
            }
        }
        ...
        private static GameObject AddKaiaVisual(Transform enemyRoot)
        {
            ...
            {
                Debug.LogWarning(
                    $"[AI Perception Demo] Player prefab not found at {PlayerPrefabPath}. " +
                    "Enemy prefab will use a fallback body collider without a character visual.");
                return null;
            }
            ...
            {
                Debug.LogWarning(
                    "[AI Perception Demo] Player_Kaia prefab has no Visual child. " +
                    "Enemy prefab will use a fallback body collider without a character visual.");
                return null;
            }
            ...
            {
                Object.DestroyImmediate(clonedWeapon);
            }
        ...
        private static bool EnsureEnemyInstance(Scene scene)
        {
            ...
            if (enemy == null)
            {
                Debug.LogError("[AI Perception Demo] Failed to instantiate the enemy prefab into the sandbox scene.");
                return false;
            }
        ...
        private static void PlaceEnemyNearPlayer(Scene scene, Transform enemy)
        {
            ...
            if (NavMesh.SamplePosition(desired, out NavMeshHit navHit, EnemyNavMeshSnapDistance, NavMesh.AllAreas))
            {
